...
pcb_t *dequeue(queue_t *queue) {
    ...
    if (scheduler_algorithm == PA) {
        ...
        while (curr != NULL) {
            if (priority_with_age(get_current_time(), curr) > highest) {
                highest = priority_with_age(get_current_time(), curr);
        ...
        while(curr != NULL) {
            if(priority_with_age(time, curr) == highest) {
                returnProcess = curr;
                if(curr == queue->head) {
        ...
        if (queue->tail == returnProcess) {
            queue->tail = prev;
        }
        returnProcess->next = NULL;
